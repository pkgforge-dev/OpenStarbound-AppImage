...
rm -v ./AppDir/bin/libsteam_api.so
cp -v ./writable-storage-and-mods.hook ./AppDir/bin/writable-storage-and-mods.hook
cp -v ./set-portable-bootconfig.hook ./AppDir/bin/set-portable-bootconfig.hook
...
if [ ${OPENSTARBOUND_INPUT:-1} -eq 1 ]; then
	...
	rm -v ./AppDir/bin/libsteam_api.so ./AppDir/bin/libdiscord_game_sdk.so
	sed -i 's|"../|"./|g' ./AppDir/bin/sbinit.config
fi
